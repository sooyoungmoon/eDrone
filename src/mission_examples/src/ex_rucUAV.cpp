...
#include "mission_lib/Goto.h"
#include "mission_lib/Target.h"
...
typedef struct str_cell
{
	...
	double x; // waypoint 좌표
	double y;
	double z; 

	double distance; // 목적지 노드까지의 거리

	bool visited; // true: 방문됨, false: 미 방문

	bool occupied; // true: 장애물 있음, false: 장애물 없음

	bool observed; // true: mental map에 포함된 cell, false: 미 관찰된 cell
...
typedef struct str_mentalMap
{
	CELL cells[AREA_WIDTH][AREA_HEIGHT];

	std::vector<CELL> visitedCells;

	std::vector<CELL> freeCells;
	
	std::vector<CELL> observedCells;	
...
std::vector<CELL> partialPath;

std::vector<CELL> coveragePath
...
std::vector<CELL> generateSubPlan(MentalMap mm)
{
...
std::vector<mission_lib::Target> path; // 무인기 자율 비행 경로


mission_lib::Arming arming_cmd;
mission_lib::Takeoff takeoff_cmd;
mission_lib::Landing landing_cmd;
mission_lib::Goto goto_cmd;



// 메시지 변수 선언
mission_lib::Target cur_target; // 무인기가 현재 향하고 있는 목적지 (경유지)
mission_lib::Target next_target; // 다음 목적지 



// 콜백 함수 
void cur_target_cb(const mission_lib::Target::ConstPtr& msg)
{
	cur_target = *msg;
...
int main(int argc, char** argv)
{
	...
	ros::ServiceClient goto_client = nh.serviceClient<mission_lib::Goto>("srv_goto");
