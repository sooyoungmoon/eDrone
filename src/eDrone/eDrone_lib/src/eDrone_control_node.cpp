...
// test 함수
void printMentalMap(Mental_Map *mental_map_ptr, const int AREA_WIDTH,
                    const int AREA_HEIGHT);
void printAltPath(std::vector<Target_Position> altPath);
void printWavefrontMap(int **mental_map, int AREA_WIDTH, int AREA_HEIGHT);

// 경로 계산 함수
std::vector<Target_Position> getAltPath(Target_Position src,
                                        Target_Position dst);
std::vector<Target_Position>
getCoveragePath(vector<geometry_msgs::Point> points, double altitude,
                double interval);
void initCell(Cell *cell_ptr, int index_x, int index_y, const int CELL_WITDH,
              const int CELL_HEIGHT, const double base_x, const double base_y);
...
//// 목적지 변수
// Target_Position target_position = { .target_seq_no = 0 }; // (2018.10.02)
// 현재 목적지 정보 (published topic)
Target_Position target_position = {
    .target_seq_no = -1}; // 현재 목적지 정보 (published topic)
int num_targets;          // 목적지 개수 (goto service마다 1 씩 증가)
bool autonomous_flight =
    false; // 자율 비행 여부  (goto service가 호출되면 true로 변경됨)
...
geometry_msgs::PoseStamped
    current_pos_local;                     // 현재 위치 및 자세 정보 (지역 좌표)
...
mavros_msgs::GlobalPositionTarget
    target_pos_global; // 목적지 위치 정보 (전역 좌표)

// (현재 목적지 정보를 응용 프로그램에 전달)
eDrone_msgs::Target cur_target; // 현재 목적지 정보 (publisher:
                                // eDrone_control_node, subscriber: 응용
                                // 프로그램)
...
std::vector<Target_Position>
    orbit_path;        // 특정 위치를 기준으로 하는 선회 비행 경로
...
eDrone_msgs::GeofenceCheck
    geofenceCheck_cmd; // 가상 울타리 확인에 사용될 요청 메시지
eDrone_msgs::NoflyZoneCheck
    noflyZoneCheck_cmd; // 비행 금지 구역 확인에 사용될 요청 메시지
...
ros::Publisher cur_target_pub; // (offboard control에 필요한) 현재 목적지 정보
                               // (도착 여부 포함)
ros::Publisher cur_phase_pub;  // 현재 무인기 임무 수행 단계 정보 (Q. 어떤 동작을
                               // 하고 있는지?)
...
vector<Target_Position> getIndirectPath(Target src, Target dest)
{
  ...
  {
    src_pt = convertGeoToENU(src.x_lat, src.y_long, src.z_alt, HOME_LAT,
                             HOME_LON, HOME_ALT);
  }
  ...
  {
    dest_pt = convertGeoToENU(dest.x_lat, dest.y_long, dest.z_alt, HOME_LAT,
                              HOME_LON, HOME_ALT);
  }
...
*/
  ...
  {
    Cell *nextCell =
        queue.front(); // queue에서 다음 cell 제거, wavefront map 확장
    if (nextCell == src_cell_ptr)
    { // wavefront map이 현재 cell까지 생성된 경우
      // map 생성 중단
      break;
    }
    ...
    for (int local_index_x = -1; local_index_x < 2; local_index_x++)
    {
      ...
      for (int local_index_y = -1; local_index_y < 2; local_index_y++)
      {
        ...
        Cell *neighbor =
            &(mental_map_ptr
                  ->grid[index_x_neighbor]
                        [index_y_neighbor]); // 현재 탐색 중인 Cell의 인접 cell

        if (waveFrontMap[index_x_neighbor][index_y_neighbor] >
            0) // 이미 label이 설정된 cell은 중복하여 설정하지 않음
        {
          continue;
        }
        ...
        if (neighbor->visited == true)
        {
          ...
          if (neighbor ==
              src_cell_ptr)
          {                   // queue에 추가된 노드가 starting node이면
            pathExist = true; // s->g까지 경로 존재 여부를 true로 설정
          }
        }
        // Case#2 장애물이 있는 Cell인 경우, 높이를 기준으로 선택적으로 queue에
        // 추가
  ...
  while (cur_cell_ptr != dst_cell_ptr)
  {
    ...
    for (int local_index_x = -1; local_index_x < 2; local_index_x++)
    {
      ...
      if (index_x_neighbor < 0 || index_x_neighbor > AREA_WIDTH)
      {
        continue;
      }
      ...
      for (int local_index_y = -1; local_index_y < 2; local_index_y++)
      {
        ...
        if (index_y_neighbor < 0 || index_y_neighbor > AREA_HEIGHT)
        {
          continue;
        }
        ...
        neighbor_cell_ptr =
            &(mental_map_ptr->grid[index_x_neighbor][index_y_neighbor]);
        ...
        if (min_label < 0 ||
            (waveFrontMap[index_x_neighbor][index_y_neighbor] <
             min_label)) // 이웃 cell 들 중 label 값이 가장 큰 cell 탐색
        {
          min_label = waveFrontMap[index_x_neighbor][index_y_neighbor];
          neighbor_min_label =
              &mental_map_ptr->grid[index_x_neighbor][index_y_neighbor];
...
void initCell(Cell *cell_ptr, int index_x, int index_y, const int CELL_WIDTH,
              const int CELL_HEIGHT, const double base_x, const double base_y)
{
...
void checkNoflyZoneCells(Mental_Map *mental_map)
{
       ...
       it != nfZones.noflyZones.end(); it++)
  {
    ...
    // 위 범위에 속한 cell들에 대해서만 isInside() 함수를 호출하여 비행금지구역
    // 포함 여부 계산
    // (연산량 감소)
    ...
    for (vector<Target>::iterator it = nfZone.noflyZone_pts.begin();
         it != nfZone.noflyZone_pts.end(); it++)
    {
      Target boundary_point = *it;
      Point point;
      //                   cout << "boundary point#" << pcnt++ << "(" <<
      //                   boundary_point.x_lat <<", " << boundary_point.y_long
      //                   << ", " << boundary_point.z_alt << ")" << endl;

      if (nfZone.noflyZone_ref_system == "WGS84")
      {
        // cout << " boundary_point.ref_system == WGS84" << endl;
        point =
            convertGeoToENU(boundary_point.x_lat, boundary_point.y_long,
                            boundary_point.z_alt, HOME_LAT, HOME_LON, HOME_ALT);
      }
      else if (nfZone.noflyZone_ref_system == "ENU")
      {
        // cout << " boundary_point.ref_system == ENU" << endl;
...
{ // 주어진 cell이 Noflyzone에 속하는 지 판단
       ...
       it != nfZones.noflyZones.end(); it++)
  {
    ...
    for (vector<Target>::iterator it = noflyZone_pts.begin();
         it != noflyZone_pts.end(); it++)
    {
      Target point = *it;
      ...
      {
        boundary_point = convertGeoToENU(point.x_lat, point.y_long, point.z_alt,
...
void printAltPath(std::vector<Target_Position>
                      altPath) // (2018.11.24) printPath로 이름 변경 요
{
  //  대체 path 정보를 화면에 출력
  cout << "eDrone_control_node: printAltPath() " << endl;
  int cnt = 0;

  for (std::vector<Target_Position>::iterator it = altPath.begin();
       it != altPath.end(); ++it)
  {
    cnt++;
...
std::vector<Target_Position> getCoveragePath(vector<eDrone_msgs::Target> points,
                                             double altitude, double interval)
{
  ...
  for (vector<Target>::iterator it = points.begin(); it != points.end(); it++)
  {
    ...
    // start - NoflyZoneCheck (20190110)

    for (vector<NoflyZone>::iterator it = nfZones.noflyZones.begin();
         it != nfZones.noflyZones.end();
         it++) // if entry_point is inside a noflyZone, return empty path
    {
      NoflyZone nfz = *it;
      ...
      if (isInside(pt, poly_area) == true)
      {
        continue; // if a boundary point is inside a noflyZone, it cannot become
                  // the entry_exit point
      }
    }
    //// end - NoflyZoneCheck (20190110)

    double distance = sqrt(
        pow((double)current_pos_local.pose.position.x - (double)point.x_lat,
            (double)2) +
        pow((double)current_pos_local.pose.position.y - (double)point.y_long,
            (double)2));
  ...
  //// (2019.01.10)
  ///  start (current_point -> entry point) path computation
  ...
  Target dest;
  dest.ref_system = "ENU";
  dest.x_lat = entry_point.x_lat;
  dest.y_long = entry_point.y_long;
  dest.z_alt = entry_point.z_alt;
  ...
  */
  ...
  cout << " (src cell) index_x:" << src_cell_ptr->index_x << ", index_y: "
       << src_cell_ptr->index_y << "x: " << src_cell_ptr->x << "y: " << src_cell_ptr->y << endl;
  ...
  cout << " (dst cell) index_x:" << dst_cell_ptr->index_x << ", index_y: "
       << dst_cell_ptr->index_y << "x: " << dst_cell_ptr->x << "y: " << dst_cell_ptr->y << endl;
  ...
  while (cur_cell_ptr != dst_cell_ptr)
  {
    ...
    for (int local_index_x = -1; local_index_x < 2; local_index_x++)
    {
      ...
      for (int local_index_y = -1; local_index_y < 2; local_index_y++)
      {
        ...
        if (max_label < 0 ||
            (waveFrontMap[index_x_neighbor][index_y_neighbor] >
             max_label)) // 이웃 cell 들 중 label 값이 가장 큰 cell 탐색
        {
          max_label = waveFrontMap[index_x_neighbor][index_y_neighbor];
          neighbor_max_label =
              &mental_map_ptr->grid[index_x_neighbor][index_y_neighbor];
...
// std::vector<Target_Position> getCoveragePath(vector<geometry_msgs::Point>
// points, double altitude, double interval)
...
void updateMap(Mental_Map *mental_map_ptr, int curCell_x, int curCell_y)
{
  ...
  // double rel_altitude_real = checkPosition_cmd.response.altitude -
  // checkPosition_cmd.response.z; //

  // mental_map_ptr->grid[curCell_x][curCell_y].rel_altitude_real =
  // rel_altitude_real;
  // ROS_INFO ("updateMap(): relative altitude is %lf meters",
  // rel_altitude_real);
  ...
  for (int local_index_x = (-1) * SENSING_RANGE; local_index_x < SENSING_RANGE;
       local_index_x++) // 현재 cell을 기준으로 8개 방향의 장애물 확인 &
                        // mental_map  확장

  // for (int local_index_x = (-1) * SENSING_RANGE; local_index_x
  // <SENSING_RANGE; local_index_x++ ) // 현재 cell을 기준으로 8개 방향의 장애물
  // 확인 & mental_map  확장
  {
    ...
    if (index_x < 0 ||
        index_x >
            AREA_WIDTH - 1) // x,y 인덱스가 mental map 범위를 벗어나면 continue
    {
      continue;
    }

    for (int local_index_y = (-1) * SENSING_RANGE;
         local_index_y < SENSING_RANGE; local_index_y++)
    {
      ...
      if (observedCell->occupied == true)
      {
        ...
        cout << " (index_x, index_y): (" << index_x << ", " << index_y << ")"
             << endl;
        cout << "  (index_x, index_y): (" << observedCell->index_x << ", "
             << observedCell->index_y << ")" << endl;
...
void ref_system_conversion_test()
{
  ...
  Point point = convertGeoToENU(HOME_LAT, HOME_LON, HOME_ALT, HOME_LAT,
                                HOME_LON, HOME_ALT);

  // Point point = convertGeoToENU(36.3847751, 127.3689272, HOME_ALT, HOME_LAT,
  // HOME_LON, HOME_ALT );
  cout << "(" << HOME_LAT << ", " << HOME_LON << ", " << HOME_ALT
       << ") =  " << endl;
  ...
  point = convertGeoToENU(36.3847732, 127.3661727, HOME_ALT, HOME_LAT, HOME_LON,
                          HOME_ALT);

  cout << "(" << 36.3847732 << ", " << 127.3661727 << ", " << HOME_ALT
       << ") =  " << endl;
  ...
  GeoPoint geoPoint =
      convertENUToGeo(0.0, 0.0, 0.0, HOME_LAT, HOME_LON, HOME_ALT);

  cout << "(0, 0, 0) =  " << endl;
  cout << fixed;
  cout << "(" << geoPoint.latitude << ", " << geoPoint.longitude << ", "
       << geoPoint.altitude << ") " << endl;
}
...
void pos_cb_local(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
  ...
  distance_home =
      current_pos_local.pose.position.x * current_pos_local.pose.position.x +
      current_pos_local.pose.position.y * current_pos_local.pose.position.y;
  ...
  {
    ROS_INFO(
        "control_node - pos_cb_local(): current position is out of geofence ");
  ...
  */
  ...
  {
    // cout << "TAKEOFF is ongoing: takeoff_altitude is " << takeoff_altitude <<
    // endl;
  ...
  //	if (phase.compare("GOTO")==0)
  {
        ...
        (target_position.is_global == false))
    {
      ...
      if (((target_position.pos_local.x - DIST_RANGE) <
           current_pos_local.pose.position.x) &&
          (current_pos_local.pose.position.x <
           (target_position.pos_local.x + DIST_RANGE)))
      {

        if (((target_position.pos_local.y - DIST_RANGE) <
             current_pos_local.pose.position.y) &&
            (current_pos_local.pose.position.y <
             (target_position.pos_local.y + DIST_RANGE)))
        {

          if (((target_position.pos_local.z - 0.5) <
               current_pos_local.pose.position.z) &&
              (current_pos_local.pose.position.z <
               (target_position.pos_local.z + 0.5)))
          {
...
void pos_cb_global(const sensor_msgs::NavSatFix::ConstPtr &msg)
{
  ...
  if (phase.compare("GOTO") == 0)
  {
    ...
    if ((target_position.reached == false) &&
        (target_position.is_global == true))
    {
      // 목적지 도착 여부 확인
      double distance;

      if (((target_position.pos_global.latitude - 0.0001) <
           current_pos_global.latitude) &&

          (current_pos_global.latitude <
           (target_position.pos_global.latitude + 0.0001)))
      {

        if (((target_position.pos_global.longitude - 0.0001) <
             current_pos_global.longitude) &&
            (current_pos_global.longitude <
             (target_position.pos_global.longitude + 0.0001)))
        {

          if (((target_position.pos_global.altitude - 0.5) <
               current_pos_global.longitude) &&
              (current_pos_global.longitude <
               (target_position.pos_global.longitude + 0.5)))
          {
            target_position.reached = true;
...
void noflyZones_cb(const eDrone_msgs::NoflyZones::ConstPtr &msg)
{
  ...
  /*
          int cnt = 0;
          for ( vector<NoflyZone>::iterator it = nfZones.noflyZones.begin(); it
     != nfZones.noflyZones.end(); it++)
          {
                  NoflyZone nfZone = *it;
                  cout << "NoflyZone#" <<cnt << endl;

                  for (vector<Target>::iterator it =
     nfZone.noflyZone_pts.begin(); it!=nfZone.noflyZone_pts.end(); it++)
                  {
                          Target point = *it;
                          cout << "point#" << cnt << "(" << point.x_lat <<", "
     << point.y_long << ", " << point.z_alt << ")" << endl;
                  }
...
bool srv_arming_cb(eDrone_msgs::Arming::Request &req,
                   eDrone_msgs::Arming::Response &res)
{
...
bool srv_takeoff_cb(eDrone_msgs::Takeoff::Request &req,
                    eDrone_msgs::Takeoff::Response &res)
{
  ...
  ROS_INFO(" HOME_ALT: %lf, req.takeoff_altitude: %lf", HOME_ALT,
           req.takeoff_altitude);
  takeoff_cmd.request.latitude =
      HOME_LAT; // 자동으로 home position 값을 얻어 와서 설정되도록 변경 필요
...
bool srv_landing_cb(eDrone_msgs::Landing::Request &req,
                    eDrone_msgs::Landing::Response &res)
{

  ROS_INFO("Landing request received\n");
  //// 서비스 요청 메시지 필드 설정

  landing_cmd.request.altitude = 5;
...
bool srv_modeChange_cb(eDrone_msgs::ModeChange::Request &req,
                       eDrone_msgs::ModeChange::Response &res)
{

  std::cout << "srv_modeChange_cb(): change the mode to " << req.modeChange_mode
            << endl;
...
bool srv_rtl_cb(eDrone_msgs::RTL::Request &req,
                eDrone_msgs::RTL::Response &res)
{
...
bool srv_goto_cb(eDrone_msgs::Goto::Request &req,
                 eDrone_msgs::Goto::Response &res)
{
  ...
  cout << "req.goto_point.x_lat: " << req.goto_point.x_lat
       << ", req.goto_point.y_long: " << req.goto_point.y_long << endl;
  // sleep(5); // (10.02) test
  ...
  if (req.goto_ref_system.compare("WGS84") == 0) // 전역 좌표인 경우
  {
    ...
    // ENU로 좌표변환
    Point point =
        convertGeoToENU(req.goto_point.x_lat, req.goto_point.y_long,
                        req.goto_point.z_alt, HOME_LAT, HOME_LON, HOME_ALT);
  ...
  else if (req.goto_ref_system.compare("ENU") == 0)
  {
    ...
    // WGS84로 좌표 변환
    GeoPoint geoPoint =
        convertENUToGeo(req.goto_point.x_lat, req.goto_point.y_long,
                        req.goto_point.z_alt, HOME_LAT, HOME_LON, HOME_ALT);
  ...
  double distance_to_home =
      sqrt(pow((double)target_position.pos_local.x, (double)2) +
           pow((double)target_position.pos_local.y, (double)2));

  if (distance_to_home > geofence.geofence_radius)
  {
    cout << "control_node-  goto_cb(): target position is outside of the "
            "geofence!! ("
         << geofence.geofence_radius << " m" << endl;
  ...
  if (geofenceCheck_client.call (geofenceCheck_cmd) == true)
  {
          ...
          if (geofenceCheck_cmd.response.value == true )
          {
                  if (geofenceCheck_cmd.response.violation == true)
                  {
                          ...
                          ROS_INFO("eDrone_control_node: Goto service rejected:
geofence violation!\n");
                          res.value = false;
                          return true;
                  }
  ...
  */
       ...
       it != nfZones.noflyZones.end(); it++)
  {
    ...
    // cout << "noflyZoneCheck_cmd.request.noflyZoneCheck_dest.ref_system:" <<
    // noflyZoneCheck_cmd.request.noflyZoneCheck_dest.ref_system << endl;
  ...
  else if (noflyZoneCheck_cmd.response.result == "PATH_OVERLAP")
  {
    ...
    // phase = "GOTO"; // (2018.11.28) to assure the completion of Goto API
    // before handling the next API call
    res.value = true;
  }
  else
  {
    ...
    target_position.reached = false;
    path.push_back(target_position);
    // target_position.target_seq_no++; // target_position 에 실제로 정보를
    // 저장>할 때 sequ_no 증가
  }
...
bool srv_gotoPath_cb(eDrone_msgs::GotoPath::Request &req,
                     eDrone_msgs::GotoPath::Response &res)
{
  ...
  {
    cout << " gotoPath_cb(): req.gotoPath_ref_system == 'WGS84' " << endl;
    Target src;
    src.ref_system = "ENU"; // src initialization

    Target dest;
    dest.ref_system = "ENU";

    Target prev_target; // previous target in each loop within the following for
                        // loops
    prev_target.ref_system = "ENU";
    prev_target.x_lat = current_pos_local.pose.position.x;
    prev_target.y_long = current_pos_local.pose.position.y;
    prev_target.z_alt = current_pos_local.pose.position.z;

    for (vector<Target>::iterator it = req.gotoPath_pts.begin();
         it != req.gotoPath_pts.end(); it++)
    {
      ...
      src.x_lat = prev_target.x_lat;
      src.y_long = prev_target.y_long;
      src.z_alt = prev_target.z_alt;
           ...
           it != nfZones.noflyZones.end(); it++)
      {
        ...
        noflyZoneCheck_cmd.request.noflyZoneCheck_dest.ref_system =
            dest.ref_system; // req.goto_ref_system;
        noflyZoneCheck_cmd.request.noflyZoneCheck_dest.x_lat = target.x_lat;
        noflyZoneCheck_cmd.request.noflyZoneCheck_dest.y_long = target.y_long;
        noflyZoneCheck_cmd.request.noflyZoneCheck_dest.z_alt = target.z_alt;
  ...
  {
    cout << " gotoPath_cb(): req.gotoPath_ref_system == 'ENU' " << endl;
         ...
         it != req.gotoPath_pts.end(); it++)
    {
      ...
      // WGS84로 좌표변환
      GeoPoint geoPoint =
          convertENUToGeo(target.x_lat, target.y_long, target.z_alt, HOME_LAT,
                          HOME_LON, HOME_ALT);
      ...
      cout << " target (" << target_position.pos_local.x << ", " << target_position.pos_local.y << ") - dist: " << distance_to_home << endl;

      if (distance_to_home > geofence.geofence_radius)
      {
        cout << "distance_to_home is larger than geofence.geofence_radius("
             << geofence.geofence_radius << ")" << endl;
        cout << "control_node-  gotoPath_cb(): target position is outside of the geofence!!" << endl;
        res.value = false; // 목적지가 비행금지구역 내에 있으면 pass
        continue;
      }
      ...
      else
      {
        ...
        // (0319)
        cout << "target (" << target_position.pos_local.x << ", " << target_position.pos_local.y << ") was added to the path" << endl;
...
bool srv_surveyArea_cb(eDrone_msgs::SurveyArea::Request &req,
                       eDrone_msgs::SurveyArea::Response &res)
{
  bool result = false;
  ...
  if (req.surveyArea_ref_system == "ENU")
  {
    ...
    for (vector<Target>::iterator it = req.surveyArea_pts.begin();
         it != req.surveyArea_pts.end(); it++)
    {
      Target target = *it;
      ...
      if (distance_to_home > geofence.geofence_radius)
      {
        cout << "control_node - surveyArea_cb(): target Area is outside of the "
                "geofence!!"
             << endl;
        res.value = false;
        return true;
    ...
    coveragePath = getCoveragePath(req.surveyArea_pts, req.surveyArea_altitude,
                                   req.surveyArea_interval);
  }
  else if (req.surveyArea_ref_system == "WGS84")
  {
         ...
         it != req.surveyArea_pts.end(); it++)
    {
      ...
      // ENU로 좌표변환
      Point point = convertGeoToENU(target.x_lat, target.y_long, HOME_ALT,
                                    HOME_LAT, HOME_LON, HOME_ALT);
    ...
    coveragePath = getCoveragePath(surveyArea_pts, req.surveyArea_altitude,
                                   req.surveyArea_interval);
  }

  printAltPath(coveragePath);

  for (vector<Target_Position>::iterator it = coveragePath.begin();
       it != coveragePath.end(); it++)
  {
    Target_Position target_position = *it;
    target_position.reached = false;
    path.push_back(target_position);
                  ...
                  eDrone_msgs::Orbit::Response &res)
{
  ...
  if (req.orbit_ref_system == "ENU")
  {
    ...
    double inclination = (req.orbit_center.y_long - cur_position.y) /
                         (req.orbit_center.x_lat - cur_position.x);
...

{
  ...
  // publisher 초기화
  pos_pub_local = nh.advertise<geometry_msgs::PoseStamped>(
      "mavros/setpoint_position/local", 10);
  pos_pub_global = nh.advertise<mavros_msgs::GlobalPositionTarget>(
      "mavros/setpoint_raw/global", 10);
  cur_target_pub =
      nh.advertise<eDrone_msgs::Target>("eDrone_msgs/current_target", 10);
  cur_phase_pub =
      nh.advertise<eDrone_msgs::Phase>("eDrone_msgs/current_phase", 10);

  // subscriber 초기화
  state_sub = nh.subscribe<mavros_msgs::State>("mavros/state", 10, state_cb);
  pos_sub_local = nh.subscribe<geometry_msgs::PoseStamped>(
      "mavros/local_position/pose", 10, pos_cb_local);
  pos_sub_global = nh.subscribe<sensor_msgs::NavSatFix>(
      "mavros/global_position/global", 10, pos_cb_global);
  home_sub = nh.subscribe<mavros_msgs::HomePosition>(
      "mavros/home_position/home", 10, homePosition_cb);
  noflyZones_sub = nh.subscribe<eDrone_msgs::NoflyZones>(
      "eDrone_msgs/noflyZones", 10, noflyZones_cb);
  geofence_sub = nh.subscribe<eDrone_msgs::Geofence>("eDrone_msgs/geofence", 10,
                                                     geofence_cb);
  ...
  modeChange_srv_server =
      nh.advertiseService("srv_modeChange", srv_modeChange_cb);
  ...
  surveyArea_srv_server =
      nh.advertiseService("srv_surveyArea", srv_surveyArea_cb);
  ...
  // noflyZoneSet_srv_server = nh.advertiseService("srv_noflyZoneSet",
  // srv_noflyZoneSet_cb);

  // noflyZoneCheck_srv_server = nh.advertiseService("srv_noflyZoneCheck",
  // srv_noflyZoneCheck_cb);

  // noflyZoneReset_srv_server = nh.advertiseService("srv_noflyZoneReset",
  // srv_noflyZoneReset_cb);

  //// 서비스 클라이언트 선언

  arming_client = nh.serviceClient<mavros_msgs::CommandBool>(
      "mavros/cmd/arming"); // service client 선언
  takeoff_client = nh.serviceClient<mavros_msgs::CommandTOL>(
      "mavros/cmd/takeoff"); // 서비스 클라이언트 선언
  landing_client = nh.serviceClient<mavros_msgs::CommandTOL>(
      "mavros/cmd/land"); // 서비스 클라이언트 선언
  noflyZoneCheck_client =
      nh.serviceClient<eDrone_msgs::NoflyZoneCheck>("srv_noflyZoneCheck");
  geofenceCheck_client =
      nh.serviceClient<eDrone_msgs::GeofenceCheck>("srv_geofenceCheck");
  ...
  while (ros::ok())
  {
    ...
    {
      // cout << "eDrone_control_node: publish Topic to takeoff - altitude " <<
      // takeoff_altitude <<  endl;
    ...
    {
      // 이륙 후, 또는 직전 위치 이동 명령 수행 후, 다음 명령을 대기하고 있는
      // 상태
      // 위치 이동 관련 명령이 호출되면, 경로 재설정 후 READY 상태로 전이 ->
      // 경로 비행
      ...
      {
        // cout << "eDrone_control_node: main() - phase is READY & path is
        // empty" << endl;
    ...
    else if (phase.compare("GOTO") == 0)
    {
      ...
      {
        // path에 목적지 정보가 있으면 첫 번째 데이터를 읽어 와서 target
        // position 갱신
        ...
        if (!path.empty())
        {
          ...
          // cout <<" target position: (" << target_position.pos_local.x  << ",
          // " << target_position.pos_local.y << ")" << endl;
          ...
          cout << " eDrone_control_node: main() - arrived at the target "
                  "position. path is not empty() "
               << endl;
          cout << " target_seq_no: " << cur_target.target_seq_no << endl;
          cout << " target position: (" << cur_target.x_lat << ", "
               << cur_target.y_long << ")" << endl;
    ...
    else if (phase.compare("ORBIT") == 0) // (2018.10.05)
    {
      ...
      {
        if (target_position.reached !=
            true) // 현재 목적지에 도착 전이면 해당 좌표 출판 (pub)
        {
        ...
        {
          if (!orbit_path.empty()) // 선회 비행 경로 변수에 목적지가 남아 있으면
                                   // 해당 위치로 이동
          {
            target_position = orbit_path[0];
