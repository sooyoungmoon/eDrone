...
// 토픽 publisher 선언 (내용 없음)


// 토픽 subscriber 선언
ros::Subscriber cur_target_sub;
...
ros::ServiceClient checkState_client;  
ros::ServiceClient checkPosition_client; 
ros::ServiceClient arming_client;
ros::ServiceClient takeoff_client ;
ros::ServiceClient landing_client ;
ros::ServiceClient goto_client;
ros::ServiceClient rtl_client ;
...
int main(int argc, char** argv)
{
	...
	cur_target_sub= nh.subscribe("eDrone_msgs/current_target", 10, cur_target_cb); // 

	// 서비스 서버 초기화 (내용 없음)

	// 서비스 클라이언트 초기화

	checkState_client =nh.serviceClient<eDrone_msgs::CheckState>("srv_checkState");  
  	checkPosition_client =nh.serviceClient<eDrone_msgs::CheckPosition>("srv_checkPosition"); 
	arming_client =nh.serviceClient<eDrone_msgs::Arming>("srv_arming");
	takeoff_client =nh.serviceClient<eDrone_msgs::Takeoff>("srv_takeoff");
	landing_client =nh.serviceClient<eDrone_msgs::Landing>("srv_landing");
	goto_client = nh.serviceClient<eDrone_msgs::Goto>("srv_goto");
	rtl_client = nh.serviceClient<eDrone_msgs::RTL>("srv_rtl");
	...
	// 서비스 요청
		...
		next_target.z_alt = GOTO_1_Z_ALT;
		...
		next_target.z_alt = GOTO_2_Z_ALT;
		next_target.reached = false;
		path.push_back(next_target);
		...
		next_target.z_alt = GOTO_3_Z_ALT;
		next_target.reached = false;
		path.push_back(next_target);
	...
	goto_cmd.request.x_lat = GOTO_1_X_LAT;
	goto_cmd.request.y_long = GOTO_1_Y_LONG;
	goto_cmd.request.z_alt = GOTO_1_Z_ALT;
